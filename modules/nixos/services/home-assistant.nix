...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.mosquitto = lib.mkIf (cfg.enableESPHome || cfg.enableMatter) {
      enable = true;
      listeners = [
        {
          port = cfg.ports.mosquitto;
    ...
    virtualisation.oci-containers = lib.mkIf cfg.enableVoiceAssistant {
      backend = "docker";
      containers = {
        whisper = {
    ...
    # ESPHome (using Docker for device compatibility)
    virtualisation.oci-containers.containers.esphome = lib.mkIf cfg.enableESPHome {
    ...
    # Matter Server (using Docker)
    virtualisation.oci-containers.containers.matter-server = lib.mkIf cfg.enableMatter {
    ...
    # Signal CLI (using Docker)
    virtualisation.oci-containers.containers.signal-cli-rest-api = lib.mkIf cfg.enableSignalCLI {
