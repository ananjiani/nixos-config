...
GPT-6 Astra is the default main-session coordinator model. Avoid routine Astra child agents to preserve the OpenAI pool, but allow Astra as escalation for hard, tool-heavy worker or high-stakes reviewer work. Never use Astra as a routine scout.
...
3. Worker and reviewer come from different providers/pools — never burn one pool on both sides of the same ticket. Grok 4.6 reviews work produced by Astra; Sol or Astra reviews Grok workers. Do not pair Astra and Sol across worker/reviewer because both use OpenAI.
4. Use the worker decision rule below. GPT-5.6 Sol reviews and is never a worker.
...
- GPT-6 Astra: main/worker `high`, `max` only for hardest work; reviewer `high`, `max` only for security-critical review.
...
- Escalate to Astra only for hard terminal/debug/migration work, or after
  Grok 4.6 fails. Do not use Astra for bounded routine work.
- GPT-5.6 Sol is never a worker. It reviews Grok workers; Grok 4.6 reviews
  Astra-produced work.
...
OpenAI's published coding-agent comparisons show Astra 9-43% lower estimated cost per task than Sol despite 2.5x token rates, but no comparison exists against Grok/GLM.
